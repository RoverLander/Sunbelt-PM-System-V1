      ...
      >
        ...
        <div style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column', gap: '2px' }}>
          {dayItems.slice(0, maxVisible).map(item => {
            ...
            const isOverdue = isPast(item.date) && !['Completed', 'Cancelled', 'Closed', 'Approved', 'Approved as Noted'].includes(item.status);
            ...
            return (
              ...
              >
                <Icon size={10} style={{ color: isOverdue ? 'var(--danger)' : item.color, flexShrink: 0 }} />
                <span style={{
                  fontSize: '0.65rem',
                  fontWeight: '500',
          ...
          {hasMore && (
            <div style={{ fontSize: '0.6rem', color: projectColor, textAlign: 'center', fontWeight: '600' }}>
          ...
          <div style={{
            ...
            <div style={{ padding: 'var(--space-md)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 'var(--space-sm)' }}>
                ...
                <span style={{ fontSize: '0.8125rem', fontWeight: '500', color: isPast(selectedItem.date) && !['Completed', 'Cancelled', 'Closed', 'Approved'].includes(selectedItem.status) ? 'var(--danger)' : 'var(--text-primary)' }}>
              ...
              {selectedItem.status && (
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <span style={{ padding: '2px 8px', borderRadius: '10px', fontSize: '0.75rem', fontWeight: '600', background: `${getStatusColor(selectedItem.status)}20`, color: getStatusColor(selectedItem.status) }}>
                    {selectedItem.status}
                  </span>
                </div>
              )}
