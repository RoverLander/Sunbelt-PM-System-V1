// ============================================================================
// SalesDashboard.jsx - Sales Team Dashboard (Praxis Enhanced)
// ============================================================================
// Main dashboard for sales reps and managers to track quotes and pipeline.
//
// FEATURES:
// - Weighted pipeline metrics (value × outlook%)
// - Visual sales funnel
// - Building type breakdown
// - Quote aging indicators (30-day scale)
// - PM flagging support
// - Praxis field display
// ============================================================================
...
// ============================================================================
// CONSTANTS - Praxis-Aligned Status Config
// ============================================================================
const STATUS_CONFIG = {
...
// Quote aging thresholds (working days)
const AGING_THRESHOLDS = {
  fresh: 15,    // 0-15 days = green
  aging: 25,    // 16-25 days = yellow
  stale: 30     // 26-30+ days = red
};
...
// Get aging color based on 30-day scale
const getAgingColor = (daysOld) => {
  if (daysOld === null) return 'var(--text-tertiary)';
  if (daysOld <= AGING_THRESHOLDS.fresh) return '#22c55e'; // Green
  if (daysOld <= AGING_THRESHOLDS.aging) return '#f59e0b'; // Yellow
  return '#ef4444'; // Red
};
...
// Render difficulty rating as stars
...
// ============================================================================
// BUILDING TYPE BREAKDOWN COMPONENT
// ============================================================================
...
function SalesDashboard() {
  ...
  // Check if current user is a Sales Manager (can see team data)
  const isSalesManager = user?.role === 'Sales_Manager' || user?.role === 'VP' || user?.role === 'Admin';

  // Check if user is a Sales Rep (sees only their own quotes)
    ...
    try {
      const [quotesRes, customersRes, dealersRes, salesTeamRes] = await Promise.all([
        ...
        supabase
          ...
          .eq('is_active', true)
          .order('name'),
        // Fetch sales team members (Sales Reps and Sales Managers)
        supabase
          .from('users')
  ...
  // COMPUTED VALUES
  // ==========================================================================
  // For Sales Reps, filter to only their quotes. Managers see all quotes.
  const userQuotes = useMemo(() => {
    if (isSalesRep && user?.id) {
      return quotes.filter(q => q.assigned_to === user.id);
    }
    return quotes;
  }, [quotes, isSalesRep, user?.id]);

  const metrics = useMemo(() => {
    const activeQuotes = userQuotes.filter(q => ACTIVE_STATUSES.includes(q.status));
    const wonQuotes = userQuotes.filter(q => q.status === 'won');
    const lostQuotes = userQuotes.filter(q => q.status === 'lost');
    const pmFlaggedQuotes = userQuotes.filter(q => q.pm_flagged && ACTIVE_STATUSES.includes(q.status));
    ...
    return {
      ...
      byStatus: Object.fromEntries(
        Object.keys(STATUS_CONFIG).map(status => [
          status,
          userQuotes.filter(q => q.status === status)
        ])
      )
    };
  }, [userQuotes]);
  ...
  const teamMetrics = useMemo(() => {
    if (!isSalesManager || salesTeam.length === 0) return [];
  ...
  }, [quotes, salesTeam, isSalesManager]);
  ...
  const filteredQuotes = useMemo(() => {
    return userQuotes.filter(quote => {
      ...
      // Team member filter (for Sales Managers)
  ...
  }, [userQuotes, searchQuery, statusFilter, factoryFilter, buildingTypeFilter, showPMFlaggedOnly, teamMemberFilter]);
  ...
  return (
    <div style={{ padding: '24px', maxWidth: '1400px' }}>
      ...
      <div style={{
        ...
        <div>
          <h1 style={{
            ...
            {isSalesManager ? 'Sales Dashboard' : 'My Sales Dashboard'}
          </h1>
          <p style={{
      ...
      {/* Compact Metrics Bar - Equal Distribution */}
      <div style={{
        display: 'grid',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '16px 20px', background: 'var(--bg-secondary)' }}>
          ...
          <div style={{ minWidth: 0 }}>
            <div style={{ fontSize: '0.65rem', color: 'var(--text-tertiary)', textTransform: 'uppercase', marginBottom: '2px' }}>Customers</div>
            <div style={{ fontSize: '1.1rem', fontWeight: '700', color: 'var(--text-primary)' }}>
              {customers.length}
            </div>
          </div>
        ...
        {/* PM Flagged - only show if there are flagged quotes */}
      ...
      {/* Compact Team Summary (Sales Managers only) */}
      {isSalesManager && salesTeam.length > 0 && (
        <div style={{
          background: 'var(--bg-secondary)',
          borderRadius: '12px',
          border: '1px solid var(--border-color)',
          ...
          {/* Compact Team Stats Row */}
          <div style={{
            display: 'flex',
            gap: '16px',
            flexWrap: 'wrap'
          }}>
            {teamMetrics.slice(0, 4).map(member => {
      ...
      {loading ? (
        ...
        <div style={{
          ...
          <div style={{ color: 'var(--text-tertiary)', fontSize: '0.9rem', marginBottom: '20px' }}>
            {searchQuery || statusFilter !== 'active' || factoryFilter !== 'all' || buildingTypeFilter !== 'all'
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px', flexWrap: 'wrap' }}>
                    ...
                    {quote.building_type && (
                      <span style={{
                        ...
                        padding: '2px 6px',
                        borderRadius: '4px',
                        background: `${BUILDING_TYPE_COLORS[quote.building_type] || '#6b7280'}20`,
                        color: BUILDING_TYPE_COLORS[quote.building_type] || '#6b7280',
                        fontWeight: '500'
                      }}>
                        {quote.building_type}
                      </span>
                  ...
                  <div style={{
                    ...
                    <span>{quote.factory || quote.praxis_source_factory}</span>
                    ...
                    {quote.module_count > 1 && (
                      <span>{quote.module_count} modules</span>
                    )}
                    {daysOld !== null && (
